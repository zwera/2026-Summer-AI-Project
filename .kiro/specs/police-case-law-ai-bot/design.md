...
경찰 판례·법령 AI 봇은 경찰 실무자가 사건번호나 죄명을 알지 못해도 현장 표현으로 상황을 입력하고, 사전에 정의된 목업 판례·법령을 탐색하며, 근거와 함께 결과를 이해하고 보고서 사실관계에 재사용하는 **웹 배포형 시연 애플리케이션**이다. 핵심 도메인 로직과 서버 측 웹 계층은 Python으로 구현하고, 브라우저는 HTML·CSS·JavaScript로 화면 표현과 사용자 상호작용만 담당한다. 사용자는 배포된 URL로 접속하며, 모든 법률 관련 결과는 서버 배포물에 포함된 목업 데이터셋에서만 결정적으로 산출된다.

이 설계에서 RAG는 운영형 검색·생성 시스템이 아니다. 구현 범위는 서버 배포물에 번들된 정적 fixture와 결정적 규칙을 사용해 다음 순서를 모사하는 완전한 목업이다.
...
현재 구현에는 임베딩, 벡터 데이터베이스, 외부 판례·법령 API, 외부 데이터 수집, 실제 LLM, 실시간 동기화, 법률 판단이 포함되지 않는다. 반면 Python 웹 서버를 통한 배포와 HTTP(S) 제공은 현재 범위에 포함된다. 화면에 보이는 법률 관련 값과 고정 경고·결측 문구는 모두 서버 배포물의 식별 가능한 목업 데이터 레코드 또는 표시 정책 레코드에서만 읽는다. 결정 규칙은 어떤 레코드 ID를 선택할지만 정하며 새로운 문구나 법률 결론을 생성하지 않는다.
...
- 외부 origin을 향한 호출 없이, 동일 배포의 Python 웹 서버와의 웹 앱 통신과 서버 배포물의 정적 자산만으로 전체 시연 흐름을 완료한다.
...
8. **탐색 상태 유지와 새로고침 복구**: 출처 상세를 열었다가 돌아오는 동작은 단일 브라우저 세션의 클라이언트 측 세션 메모리(영속 저장 없음)에서 보존한다. 화면 새로고침 시에는 현재 브라우저 세션 범위 내에 상황 질의·선택 판례·요약 단계·보조 필터가 존재하면 서버·영속 저장 없이 세션 메모리/클라이언트 상태 기준으로 화면 상태를 복구한다. 세션에 복구할 상태가 없으면 진입 화면과 `임시 화면 상태가 초기화되었습니다.` 안내를 표시한다. 사용자별 영속 저장은 요구 범위가 아니다.
...
- **속성 테스트 설계 권장사항**: Python 구현이 확정되어 있으므로 성숙한 Python PBT 라이브러리(예: [Hypothesis 설정 문서](https://hypothesis.readthedocs.io/en/latest/settings.html))를 구현 시 다시 확인하고, 지원되는 공식 옵션을 사용해 각 속성을 최소 100회 실행하며 재현 정보와 축소된 최소 반례를 보존하는 안을 권장한다. 최소 100회와 반례 보존은 이 설계의 테스트 계약이며 특정 라이브러리가 이미 설치되었다는 뜻이 아니다.

링크 대상의 내용을 이 문서에서 검증된 사실로 단정하거나 인용하지 않는다. 링크는 구현 시 재검증할 참고 후보이고, 위 항목의 구체적 적용 방식은 이 기능의 설계 결정이다.

### 6. 조건부 권장 구현 선택과 대안

서버·도메인 언어(Python)와 클라이언트 표현 기술(HTML·CSS·JavaScript)은 요구사항 15로 확정되어 있다. 아래 표에서 언어·속성 테스트 항목은 확정 사항이고, 웹 프레임워크 등 나머지는 구현 착수 시 평가할 **조건부 권장 후보**이며 기존 의존성이나 승인된 스택을 뜻하지 않는다. 이 문서의 계약 의사코드는 계약을 구체화하기 위한 표기이며 특정 프레임워크 API를 뜻하지 않는다.

| 관심사 | 조건부 권장 후보 | 대안 | 선택 이유 |
|---|---|---|---|
| 언어(서버·도메인) | Python + type hints·정적 타입 검사 | — | 요구사항 15.1·15.2로 확정 |
| 언어(클라이언트) | HTML·CSS + JavaScript | — | 요구사항 15.8·15.9 범위의 표현·상호작용 전용 |
| 웹 프레임워크 | 착수 시 확정(FastAPI 또는 Flask + 서버 템플릿) | Django | 요구사항이 설계 단계에 위임한 선택 |
| 상태 | 순수 reducer + selector | 명시적 상태 머신 라이브러리 | 단계 전이와 오류 전 상태를 명시적으로 검증 |
| fixture 검증 | Python 스키마·검증 라이브러리 또는 사용자 정의 validator | dataclass + 수동 검증 | 교차 참조 검증 전 구조 검증 |
| 예제 테스트 | pytest | unittest | 성숙한 생태계와 빠른 단일 실행 |
| 속성 테스트 | Hypothesis | — | Python 생태계의 성숙한 PBT 라이브러리 |
| 접근성 점검 | axe 계열 자동 점검 + 수동 키보드/스크린 리더 점검 | pa11y | 자동 규칙만으로 확인 못 하는 초점·읽기 순서를 수동 보완 |
| 서버 제공 | Python 웹 서버가 서버 렌더링 HTML과 정적 자산을 HTTP(S)로 제공 | 정적 자산 + 최소 JS 클라이언트 | 요구사항 1.11·14.7·14.8의 웹 배포·HTTP 제공 충족 |

클라이언트는 표현·상호작용 전용이며 법률 로직을 재계산하지 않으므로, 서버 렌더링 HTML과 최소 JavaScript를 전제한다. 구현의 첫 기술 결정 단계에서 배포 제약, 팀 역량, 테스트 가능성을 기준으로 웹 프레임워크·검증·테스트 도구를 확정한다. FastAPI/Flask, pytest, Hypothesis, axe 계열은 그때 채택 여부를 판단할 후보일 뿐이며(언어·속성 테스트 라이브러리 종류 확정 범위 제외), 다른 선택을 하더라도 아래 도메인 계약·목업 전용 경계·추적성 요구는 유지한다. 채택한 모든 의존성은 정확한 버전으로 고정하고 실제 사용 가능 옵션과 라이선스를 해당 시점의 공식 자료로 확인한다. 목업 동작을 위해 검색 SDK, 벡터 저장소 SDK, 외부 판례·법령 API 클라이언트 또는 외부 AI/LLM 라이브러리를 추가하지 않는다.
...
flowchart LR
    ...
    subgraph BR[브라우저 - 클라이언트]
      UI[클라이언트 웹 계층\nHTML·CSS·최소 JS: 표현·상호작용·화면 상태]
      IO[로컬 클립보드 / UTF-8 파일]
      UI --> IO
    end
    subgraph SV[Python 웹 서버 - 서버 배포물]
      WEB[서버 측 웹 계층\nHTTP 라우팅·요청 검증·응답 생성·정적 자산]
      APP[애플리케이션 계층\nReducer·Mock RAG Orchestrator]
      DOM[순수 도메인 계층\n변환·정렬·분류·검증·보고서]
      REP[Fixture Repository\n교차 참조 검증된 읽기 전용 데이터]
      VA[음성 시연 어댑터 - 사전 정의 인식 텍스트]
      WEB --> APP --> DOM --> REP
      VA --> APP
      APP --> WEB
    end
    FIX[(번들 JSON/Python fixture + 정적 자산)] --> REP
    U <--> UI
    UI <-->|HTTP S 웹 앱 통신| WEB
    EXT[(검색 엔진 / 벡터 DB / 외부 API / LLM)]
    DOM -. 연결 없음 .-x EXT
```

런타임 의존 방향은 `클라이언트_웹_계층 → Python_웹_서버 → Application → Domain ← Repository contract`로 제한하고, 서버 배포물에 번들된 fixture 어댑터만 도메인 저장소 계약을 구현한다. `FixtureRepository` 이외의 검색·데이터 공급자는 현재 런타임에 존재하지 않는다. 클라이언트의 네트워크 호출은 동일 배포의 Python 웹 서버를 향한 웹 앱 통신과 정적 자산 요청으로만 한정하고, 외부 origin을 향한 `fetch`, `XMLHttpRequest`, `WebSocket`, `EventSource`, `sendBeacon` 호출은 두지 않는다. 질의 처리 중 외부 origin을 향한 네트워크·서비스 실행 호출은 0건이며, 검색 엔진·벡터 DB·외부 판례/법령 API 같은 외부 서비스는 사용하지 않는다. 질의 처리는 서버 메모리에 로드된 fixture 조회와 순수 함수 호출로 완료하고, 클립보드 복사와 파일 다운로드만 클라이언트 측 로컬 I/O 경계에서 수행한다.
...
| 계층 | 위치 | 책임 | 금지 사항 |
|---|---|---|---|
| 클라이언트 웹 계층 | 브라우저 | 입력, 시나리오 탐색, 결과 목록/상세, 접근성, 반응형 배치, 초점·강조, 웹 앱 통신 | 법률 상태 계산·재계산, fixture 직접 조회, 결과 재정렬·재분류·합성 |
| 서버 측 웹 계층 | Python 웹 서버 | HTTP(S) 라우팅, 요청 검증, 세션 범위 처리, 응답 생성, 정적 자산 제공, 픽스처 출처정보 동봉 | 임의 문구 생성, 외부 origin 호출 |
| 애플리케이션 계층 | Python 웹 서버 | 명령 처리, RAG 상태 전이, 화면 상태 보존, 오류 전 스냅샷, 컴포넌트 조정 | 임의 문구 생성, 외부 I/O |
| 도메인 계층 | Python 웹 서버 | 질의 해석, 안정 정렬, 상태 분류, 출처 판정, 타임라인 정렬, 보고서 생성 | 네트워크, 현재 시각 의존 |
| 데이터 계층 | Python 웹 서버 | 구조·교차 참조·불변식 검증, ID 조회, 읽기 전용 projection | 검색 산식 추론, 레코드 자동 보정 |
| 로컬 I/O 경계 | 브라우저 | 클립보드 복사, UTF-8 다운로드, 음성 시연 선택 | 외부 업로드, 원격 분석, 자동 영속 저장 |

모든 도메인 함수에는 현재 시각 조회, 난수, 로케일 의존 정렬을 넣지 않는다. 날짜 비교 기준은 오직 `dataset.asOfDate`, 동순위 최종 키는 정규화된 `caseId`다.
...
### 7. 웹 배포와 외부 origin 차단

- fixture, 글꼴, 아이콘, 스키마를 서버 배포물에 포함하고 외부 CDN을 사용하지 않는다.
- 배포 시 Python 웹 서버가 서버 렌더링 HTML과 정적 자산을 HTTP(S)로 제공하며, 동일 origin 서버 통신은 허용된다. 외부 판례·법령 API, 검색 엔진, 벡터 DB 등 외부 origin에 대한 연결 없이 목업 흐름이 완료되어야 한다.
- 가능한 호스트 환경에서는 CSP `default-src 'self'; connect-src 'self'`를 적용해 동일 origin 통신만 허용하고 외부 origin 연결을 차단한다. 개발 도구의 라이브 리로드는 시연/검증 빌드에서 제거한다.
- 서비스 워커나 원격 분석 코드를 두지 않는다. 브라우저 네트워크 계측 테스트로 질의 제출부터 완료/실패까지 외부 origin 요청 0건을 확인한다(동일 origin 웹 앱 통신과 정적 자산 요청은 이 판정에서 제외한다).
...
다음은 확정된 Python 기준 구현의 계약을 기술하는 의사코드다. 표기는 판별 유니온·`Literal`·`Protocol`·dataclass 등 Python 관용을 염두에 둔 계약 표현이며, 특정 웹 프레임워크 API 또는 이미 존재하는 프로젝트 구현을 뜻하지 않는다. `Result`는 성공·오류를 구분하는 판별 유니온으로 읽는다. 필드명·구조·의미는 언어와 무관하게 그대로 유지한다.
...
- `includeInstanceCaution`은 판례 목록·상세에 `판례는 심급 및 절차 경과에 따라 결론이 달라질 수 있으므로, 상급심 판단과 확정 여부를 함께 확인해야 합니다.`라는 심급·확정 여부 확인 안내가 포함되어야 함을 뜻한다.
- 유사도 표시 화면에는 사전 정의 목업 값 고정 문구가 포함된다.
- 공개_배포 고지문과 법률_안전 고지문처럼 긴 문구는 요구사항 17.6·17.7의 상시 가시 노출을 유지하되, 좁은 화면 본문을 밀어내지 않도록 `DisplayPolicyRecord.summaryLabel`(요약 라벨 1줄, 상시 노출)과 `fullText`(전체 문구, 접근 가능한 펼침)로 나누어 표시한다. 요약 라벨 자체가 상시 가시 영역에 있으므로 상시 노출 요구를 충족하며, 전체 문구는 키보드·스크린 리더로 펼칠 수 있어야 한다.
...
interface MockDataset {
  ...
  instanceCautionNotice: "판례는 심급 및 절차 경과에 따라 결론이 달라질 수 있으므로, 상급심 판단과 확정 여부를 함께 확인해야 합니다.";
...
interface DisplayPolicyRecord {
  ...
  key: string;
  text: string;
  // 긴 고지문(공개_배포·법률_안전 고지문 등)이 좁은 화면 본문을 밀어내지 않도록,
  // 요약 라벨 1줄과 전체 문구를 분리해 접근 가능한 펼침(확장)으로 표시한다.
  // summaryLabel이 있으면 요약 라벨을 상시 노출하고 fullText는 펼침 영역에 둔다.
  // 두 필드가 없으면 text를 그대로 상시 노출한다.
  summaryLabel?: string;
  fullText?: string;
}
...
interface CaseRecord {
  ...
  relatedInstances: readonly RelatedInstanceRef[];
...
interface AppellateInformation {
  state: "PRESENT" | "정보_없음";
  // 동일 사건의 심급 체인. 각 결정은 자신의 원심(직전 하급심) 대비 관계를 기술한다.
...
`state=정보_없음`이면 decisions는 빈 배열이어야 한다. `finality=정보_없음`이면 확정·미확정 배지를 생성하지 않는다. 변경 판결은 원심(하급심) 결과와 상급심 결과를 함께 보여 주고 텍스트·아이콘으로 차이를 강조한다.
...
속성은 특정 예시 하나가 아니라 생성 가능한 모든 유효 입력에 적용되는 명세다. 아래 속성은 사전 분석 후 논리적으로 중복되는 항목을 통합한 결과다. UI의 시각적 배치, 고정 문구, 실제 브라우저 초점, 외부 origin 차단 배선은 보편 입력 속성으로 억지 변환하지 않고 뒤의 예제·통합·접근성 테스트에서 다룬다.
...
6. **외부 fallback 금지**: fixture 오류 시 외부 검색·LLM·외부 origin 호출로 보완하지 않는다.
...
진단은 오류 코드, fixture 레코드 ID, 스키마 경로만 포함한다. Python 웹 서버의 애플리케이션_로그는 요청 메타데이터와 비민감 오류 정보만 기록하고, 사용자가 입력한 원문 상황 질의·선택 텍스트·보고서 본문 같은 민감_사용자_콘텐츠는 로그·외부 전송에 포함하지 않는다. 외부_분석_전송은 두지 않으며, 클라이언트 개발용 콘솔 진단도 시연 빌드에서 기본 비활성화한다.

## Testing Strategy

### 1. 테스트 계층

이 기능은 정렬·분류·상태 전이·집합 변환처럼 입력 공간이 큰 순수 로직을 포함하므로 PBT가 적합하다. 동시에 UI 고정 문구, 초점, 외부 origin 차단 동작은 예제·통합 테스트가 더 적절하다. 두 방식을 다음처럼 함께 사용한다.

모든 테스트의 시스템 대상은 **서버 배포물에 번들된 fixture, 순수 도메인 함수, reducer, 화면 projection, Python 웹 서버 계층**으로 한정한다. 테스트에서 “목업 RAG”는 네 단계의 상태 전이와 fixture lookup 결과를 뜻한다. 실제 검색 엔진·임베딩·벡터 DB·외부 판례/법령 API·원격 음성 인식·실제 LLM을 호출하지 않으며, 그런 서비스의 sandbox나 mock server, fake client를 만들어 통합된 것처럼 시험하지도 않는다. 네트워크 테스트는 동일 origin 웹 앱 통신의 정상 동작은 허용하되, **외부 origin을 향한 의존성과 호출이 없다는 사실**을 검증한다.

1. **스키마/정적 검사**: fixture 구조, ID 참조, 금지된 외부 origin 모듈·URL·검색/벡터/외부 API/LLM 클라이언트·SDK, 정확한 안전 고지 문자열을 검사한다.
...
5. **브라우저 통합/E2E**: 대표 시연 흐름, 상태 왕복, 클립보드/다운로드, 동일 origin 웹 앱 통신으로의 흐름 완료, 질의 처리 중 외부 origin 호출 0건을 검증한다.
6. **접근성/반응형 테스트**: 자동 규칙과 수동 키보드·스크린 리더·확대·고대비 검사를 병행한다.

### 2. 속성 기반 테스트 구성

언어는 Python으로 확정되어 있으므로 성숙한 Python PBT 라이브러리(**Hypothesis 우선 검토**)를 사용하는 안을 권장하며, 구현 시 선택 버전의 공식 문서에서 필요한 생성기(strategy), 실행 횟수, seed·축소 결과 재현 기능을 확인한다. 자체 난수 생성기를 구현하지 않는다. 도구 선택과 무관하게 각 설계 속성은 **정확히 하나의 property test**로 구현하고 최소 100회 실행한다. 복잡한 상태 모델 속성(2, 39, 40)은 기본 100회에서 시작하되 CI 시간 안에서 더 높일 수 있다.

아래 코드는 확정된 프로젝트 코드나 설치된 API가 아니라 Python/Hypothesis를 채택했을 때의 테스트 계약 예시다.

각 테스트에는 다음 형식의 주석을 둔다.

```python
# Feature: police-case-law-ai-bot, Property 19: 검색 우선순위의 결정적 안정 정렬
@settings(max_examples=100)  # 실패 시 seed로 재현, 최소 반례 보존
@given(case_list_strategy)
def test_deterministic_stable_sort(cases):
    ...
```
...
- Hypothesis 외의 성숙한 Python PBT 라이브러리를 채택하면 그 라이브러리에 맞게 이 테스트 도구 예시를 갱신한다. 어떤 도구를 선택해도 속성 번호·100회 이상 실행·재현 정보와 최소 반례 보존 계약은 유지한다.
...
외부 origin 차단 테스트에서는 브라우저 컨텍스트가 동일 origin(Python 웹 서버)만 허용하도록 시작한 뒤, 질의 제출 시점부터 완료/실패까지 외부 origin을 향한 `fetch`, XHR, WebSocket, EventSource, beacon 요청 건수가 0인지 계측한다. 동일 origin 웹 앱 통신과 정적 자산 요청은 이 계측에서 제외한다. CSP(`default-src 'self'; connect-src 'self'`)가 적용된 시연 빌드에서도 같은 흐름을 실행한다.
...
- 깨끗한 환경에서 의존성을 고정 버전으로 설치하고 Python 웹 서버 기동과 정적 자산 빌드가 성공해야 한다.
- Python 웹 서버가 진입·결과 화면 route와 정적 자산 요청을 정상 처리하고, 정의되지 않은 URL은 안전 오류 응답을 반환해야 한다.
- 외부 origin 연결이 차단된 상태에서 fixture 로드, 검색, 출처, 타임라인, 보고서 다운로드가 동일 origin 웹 앱 통신만으로 완료되어야 한다.
- 서버·클라이언트 산출물에서 외부 origin URL, 원격 글꼴/아이콘, AI/API SDK, 응답·소스에 노출된 비밀값이 없는지 검사한다.
...
|---|---|---|---|---|
| 1. 목업 RAG·고지 | AppShell, MockRagOrchestrator, LegalNoticePolicy, Python 웹 서버, fixture 전용·외부 origin 없는 계층 | `RagStage`, `NoticeSurface`, `MockDataset` | P1–P3, P26 | 고정 문구, 동일 origin 웹 앱 통신·외부 origin 0건 통합 |
