...
1. **상황 입력** (`SituationInputStep`) — 자연어 서술 + 🎤 음성 입력 → `/api/chat` 호출. 하단에 `FieldManualCard`("상황 대응 간의 매뉴얼") 표시 — 이 카드 안의 빠른 상황 선택 칩을 누르면 텍스트 입력은 그대로 두고 매뉴얼(우선 조치 체크리스트)만 해당 상황에 맞게 전환됨, AI 분석 전 참고용 (`constants/taxonomy.js`의 `FIELD_MANUALS`)
